...
const Search = () => {
  ...
  const searchByTitle = async () => {
    try {
      ...
      const { data } = await axios.get(
        `http://127.0.0.1:8000/intern/search/title/?search=${selectedOption}`
      );
      setSearchResult(data);
      console.log(data);
      setLoading(false);
  ...
  const searchByDuration = async () => {
    try {
      ...
      setLoading(true);
      const { data } = await axios.get(
        `http://127.0.0.1:8000/intern/search/duration/?duration=${selectedOption}`
      );
      setSearchResult(data);
      console.log(data);
      setLoading(false);
  ...
  const searchBySkill = async () => {
    try {
      ...
      const arr = [selectedOption];
      const { data } = await axios.get(
        `http://127.0.0.1:8000/intern/search/skills/?user_skills=${arr}`
      );
      setSearchResult(data);
      console.log(data);
      setLoading(false);
